...
const addCart = asyncWrapper(async (req, res, next, session) => {
  try {
    const newCart = new Cart(req.body);
    savedCart = await newCart.save({ session });
    console.log("savedCart vale ", savedCart);
    res.status(StatusCode.OK).json(savedCart);
  } catch (err) {
    res
      .status(500)
      .json(
        CreateError(
          500,
          `Internal server error ${err} adding new Cart ${savedCart}!`
        )
      );
  }
});
